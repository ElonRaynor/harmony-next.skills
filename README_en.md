...
| `Unreleased` | Added guarded DevEco Emulator CLI launch entries: `hvd_manager.py launch-preflight` only prints a command plan with `-t <trace-name>`, while `hvd_manager.py launch` creates a bounded trace socket and starts Emulator; missing trace, HVD, Emulator, or image root returns machine-readable blocked output |
