...
- On macOS the emulator image root is commonly `~/Library/Huawei/Sdk`; the script validates it against the HVD `imageSubPath`. `launch` detaches Emulator and the trace holder, then waits for HDC, boot, and stability checks. `download-image` currently returns blocked because image downloads have only been verified through the IDE SDK Manager UI.
