...
2. 扩展 `harmony-next/scripts/reference_compat.py`：
   ...
   - `check` 时阻止这些目标外链残留
3. 重建 `harmony-next/references/INDEX.md` 与 `harmony-next/references/JsEtsAPIReference/INDEX.md`
...
27. 第十一轮继续补齐全部 `4` 次残留的高频在线 guide：
   - `user-file-uri-intro` -> `guides/用户文件URI介绍.md`
   - `time-consuming-task-overview` -> `guides/耗时任务并发场景简介.md`
   - `scenario-fusion-intelligent-filling` -> `guides/智能填充服务.md`
   - `push-apply-right` -> `guides/申请推送场景化消息权益.md`
   - `permissions-for-all-user` -> `guides/开放权限（用户授权）.md`
   - `payment-server-connect` -> `guides/云侧服务准备.md`
   - `devicesecurity-deviceverify-token` -> `guides/基于服务账号生成鉴权令牌（Device Security）.md`
   - `crypto-asym-key-generation-conversion-spec` -> `guides/非对称密钥生成和转换规格.md`
   - `arkts-wrapbuilder` -> `guides/wrapBuilder：封装全局@Builder.md`
   - `arkts-interaction-development-guide-keyboard` -> `guides/支持键盘输入事件.md`
   - `arkts-environment` -> `guides/Environment：设备环境查询.md`
   - `ability-recover-guideline` -> `guides/UIAbility备份恢复.md`
28. 第十一轮后重新执行 `generate/check/audit` 与单元测试：
   - `generate` 实际新增重写 23 个 guide-link 文件
   - `check` 与 `audit` 通过
   - `python3 -m unittest discover -s harmony-next/tests -p 'test_*.py' -v` 通过
   - 仓库内剩余 HarmonyOS 在线 guide slug 最高频次已降到 `3` 次，`4` 次及以上的高频头部已全部清空
...
- 截至第十一轮，`4` 次及以上的高频在线 guide slug 已全部清空；后续若继续扩面，优先处理当前 `3` 次残留项，并沿用“先抓官方正文、再补本地 guide、最后接映射”的流程推进。
