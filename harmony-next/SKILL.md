...
- If a wrapper is blocked, misleading, or missing a useful workflow, recommend the scenario-specific GitHub issue form from the script's `feedback` JSON field or `ISSUE_GUIDE.md`; preserve structured fields and follow the redaction rules.
...
- For blocked results, confusing rule output, or unsupported DevEco/UxTestService versions, ask the user to open a repository issue using the `feedback` section in `summary.json` / `report.md`; never attach raw screenshots, raw layout JSON, full logs, or marked images without verified redaction.
